...
class MBConvBlock3D(nn.Module):
    ...
    def __init__(self, block_args, global_params):
        ...
        p = ((k - 1) // 2)  # This will maintain the input size if stride is 1
    ...
    def forward(self, inputs, drop_connect_rate=None):
        """
        Forward pass of the Mobile Inverted Residual Bottleneck Block.

        Args:
            inputs (torch.Tensor): The input tensor.
            drop_connect_rate (float, optional): The drop connect rate (between 0 and 1).
        ...
        if self.has_se:
            # Squeeze and Excitation layer operations
...
class EfficientNet3D(nn.Module):
    ...
    def extract_features(self, inputs):
        ...
        print("After stem, input tensor size:", x.size())
        ...
        for idx, block in enumerate(self._blocks):
            ...
            print("After block {}, input tensor size:".format(idx), x.size())

        # Head
        x = self._swish(self._bn1(self._conv_head(x)))
        print("After head, input tensor size:", x.size())
